...
class _UsersPageState extends State<UsersPage> {
  ...
  Future<void> _exportCsv() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم نسخ CSV إلى الحافظة')));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(actions: [
        IconButton(icon: Icon(Icons.copy), onPressed: _exportCsv, tooltip: 'تصدير CSV'),
      ...
      body: _loading
          ...
          : Padding(
              ...
              child: Column(
                children: [
                  Text('إدارة المستخدمين', style: Theme.of(context).textTheme.headline6),
                  SizedBox(height: 8),
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) => UserTile(
                        ...
                        onEdit: () => _showEditor(user: _users[i]),
                        onDelete: () => _delete(_users[i].id),
                      ),
                    ),
                  ),
                ],
